...
fun HomePager(mainInnerPadding: PaddingValues, isActive: Boolean) {
    ...
    val quoteTitle = if (homeHitokotoEnabled) homeQuote.ifBlank { MLang.Home.Title } else MLang.Home.Title
            ...
            item {
                Column(
                    ...
                    HomeTrafficSection(
                        ...
                        onProxyModeClick = { hapticFeedback.performHapticFeedback(HapticFeedbackType.VirtualKey) },
                        onProxyModeSelected = { mode -> hapticFeedback.performHapticFeedback(HapticFeedbackType.VirtualKey); homeViewModel.switchProxyMode(mode) },
                    )
                    ...
                    Column(verticalArrangement = Arrangement.spacedBy(UiDp.dp16)) {
                        ...
                        HomeSpeedChartSection(homeViewModel = homeViewModel, isRunning = isRunning, onClick = { navigator.push(Route.TrafficStatistics) })
                        if (isRunning) { HomeTopologyChartSection(homeViewModel = homeViewModel, onClick = { navigator.push(Route.Connection) }) }
                    }
                }
            }
 ...
 */
@Composable
private fun HomeTrafficSection(homeViewModel: HomeViewModel, isRunning: Boolean, profileName: String?, tunnelMode:Mode?, currentProfileId: String?, profileOptions: List<Profile>, controlState: HomeProxyControlState, proxyMode: ProxyMode, isRemoteController: Boolean, isEnabled: Boolean, onClick: () -> Unit, onProfileNameClick: () -> Unit, onProfileSelected: (String) -> Unit, onTunnelModeClick: () -> Unit, onTunnelModeSelected: (Mode) -> Unit, onProxyModeClick: () -> Unit, onProxyModeSelected: (ProxyMode) -> Unit) {
    val trafficData by homeViewModel.trafficData.collectAsStateWithLifecycle()
    TrafficDisplay(trafficNow = if (isRunning) trafficData else TrafficData.zero, profileName = profileName, tunnelMode = tunnelMode, currentProfileId = currentProfileId, profileOptions = profileOptions, controlState = controlState, proxyMode = proxyMode, isRemoteController = isRemoteController, isEnabled = isEnabled, onClick = onClick, onProfileNameClick = onProfileNameClick, onProfileSelected = onProfileSelected, onTunnelModeClick = onTunnelModeClick, onTunnelModeSelected = onTunnelModeSelected, onProxyModeClick = onProxyModeClick, onProxyModeSelected = onProxyModeSelected)
}
