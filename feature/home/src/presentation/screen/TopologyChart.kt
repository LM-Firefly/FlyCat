...
import com.github.yumelira.yumebox.core.util.resolveProxyChainOrder
...
fun TopologyChart(connections: List<ConnectionInfo>, onClick: () -> Unit, modifier: Modifier = Modifier) {
    ...
    val topologyKey = remember(connections) { connections.map { c -> "${c.id}:${c.chains.joinToString(",")}" }.hashCode() }
    ...
    if (sankeyData.nodes.isEmpty()) {
        Box(modifier = modifier.height(200.dp).fillMaxWidth()) {
            Column(modifier = Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.CenterHorizontally) {
                Text(text = "No active connections. Tap to open connection details.", style = TextStyle(fontSize = 14.sp, color = colorScheme.onSurface.copy(alpha = 0.7f)))
                Spacer(modifier = Modifier.height(8.dp))
                Text(text = "Open connection page", style = TextStyle(fontSize = 12.sp, color = colorScheme.primary), modifier = Modifier.clickable { onClick() })
            }
        }
        return
    }
...
private fun processConnections(connections: List<ConnectionInfo>): SankeyData {
    ...
    connections.forEachIndexed { connectionIndex, conn ->
        ...
        val chains = resolveProxyChainOrder(conn.chains)
