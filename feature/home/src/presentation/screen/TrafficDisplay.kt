        ...
        Box {
            Surface(color = MiuixTheme.colorScheme.primary.copy(alpha = opacity.subtle), shape = RoundedCornerShape(50), modifier = Modifier.heightIn(min = 28.dp).widthIn(max = 200.dp).clickable(interactionSource = remember { MutableInteractionSource() }, indication = null, role = Role.Button, onClick = { onProfileNameClick(); if (enabledProfiles.isNotEmpty()) { showProfilePopup = true } })) {
                Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.padding(horizontal = 12.dp, vertical = 4.dp), horizontalArrangement = Arrangement.spacedBy(spacing.space6)) {
                    Text(text = profileName ?: MLang.Home.Traffic.NoProfile, style = MiuixTheme.textStyles.footnote1.copy(fontSize = 12.sp, fontWeight = FontWeight.Bold), color = MiuixTheme.colorScheme.primary, modifier = Modifier.weight(1f, fill = false).basicMarquee(iterations = Int.MAX_VALUE, velocity = 30.dp), maxLines = 1, softWrap = false)
                }
            }
...
private fun ProxyTypeCapsule(proxyMode: ProxyMode, isEnabled: Boolean, onProxyModeClick: () -> Unit = {}, onProxyModeSelected: (ProxyMode) -> Unit = {}) {
    ...
    Box {
        Surface(color = primary.copy(alpha = opacity.subtle), shape = RoundedCornerShape(50), modifier = Modifier.height(componentSizes.statusCapsuleHeight).clickable(enabled = isEnabled, interactionSource = interactionSource, indication = null, role = Role.Button, onClick = {onProxyModeClick(); showPopup = true })) {
            Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.padding(horizontal = spacing.space12), horizontalArrangement = Arrangement.spacedBy(spacing.space6)) {
                Icon(imageVector = when (proxyMode) { ProxyMode.Tun -> Yume.PlaneTakeoff; ProxyMode.RootTun -> Yume.Tun; ProxyMode.Http -> Yume.Wifi }, contentDescription = null, tint = primary, modifier = Modifier.size(spacing.space12))
                Text(text = when (proxyMode) { ProxyMode.Tun -> MLang.Home.ProxyMode.Vpn; ProxyMode.RootTun -> MLang.Home.ProxyMode.Tun; ProxyMode.Http -> MLang.Home.ProxyMode.Http }, style = MiuixTheme.textStyles.footnote1.copy(fontSize = 12.sp, fontWeight = FontWeight.Bold), color = primary)
            }
        }
        OverlayListPopup(show = showPopup, alignment = PopupPositionProvider.Align.BottomStart, onDismissRequest = { showPopup = false }) {
            ListPopupColumn {
                proxyModeOptions.forEachIndexed { index, mode ->
                    DropdownImpl(text = when (mode) { ProxyMode.Tun -> MLang.Home.ProxyMode.Vpn; ProxyMode.RootTun -> MLang.Home.ProxyMode.Tun; ProxyMode.Http -> MLang.Home.ProxyMode.Http }, optionSize = proxyModeOptions.size, isSelected = mode == proxyMode, onSelectedIndexChange = { showPopup = false; onProxyModeSelected(mode) }, index = index)
                }
            }
        }
    }
...
private fun TunnelState.Mode?.toDisplayName(): String =
    when (this) {
        TunnelState.Mode.Direct -> MLang.Home.TunnelMode.Direct
        TunnelState.Mode.Global -> MLang.Home.TunnelMode.Global
        TunnelState.Mode.Rule -> MLang.Home.TunnelMode.Rule
        else -> "--"
    }
