...
fun MoeHomePage(
    ...
    val sidebarIcons = remember(onOpenDashboard) {
        listOf(
            MoeSidebarIconItem(Yume.Zashboard) { onOpenDashboard?.invoke() },
            MoeSidebarIconItem(Yume.ArrowDownUp) { handlePageChange(1) },
            MoeSidebarIconItem(Yume.PackageCheck) { handlePageChange(2) },
            MoeSidebarIconItem(Yume.Bolt) { handlePageChange(3) },
        )
    }
