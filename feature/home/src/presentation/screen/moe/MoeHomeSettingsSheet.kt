        ...
        LazyColumn(modifier = Modifier.fillMaxWidth().padding(bottom = spacing.space16)) {
            item {
                Card(modifier = Modifier.fillMaxWidth().padding(bottom = spacing.space12)) {
                    Column {
                        PreferenceSwitchItem(title = MLang.AppSettings.Interface.ClassicHomeTitle, checked = draftClassicHomeEnabled, onCheckedChange = { draftClassicHomeEnabled = it })
                        if (draftClassicHomeEnabled) { PreferenceSwitchItem(title = MLang.AppSettings.Interface.HitokotoTitle, summary = MLang.AppSettings.Interface.HitokotoSummary, checked = draftHomeHitokotoEnabled, onCheckedChange = { draftHomeHitokotoEnabled = it }) }
                        if (!draftClassicHomeEnabled) { PreferenceSwitchItem(title = MLang.AppSettings.Interface.SidebarExpandedTitle, checked = draftSidebarExpanded, onCheckedChange = { draftSidebarExpanded = it }) }
                    }
                }
            }
            val showQuoteFields = !draftClassicHomeEnabled || draftHomeHitokotoEnabled
            if (showQuoteFields) {
                item { TextField(value = draftQuote, onValueChange = { draftQuote = it }, label = MLang.AppSettings.Interface.HomeQuoteTitle, useLabelAsPlaceholder = true, maxLines = 2, modifier = Modifier.fillMaxWidth().padding(bottom = spacing.space12)) }
                item { TextField(value = draftQuoteAuthor, onValueChange = { draftQuoteAuthor = it }, label = MLang.AppSettings.Interface.HomeQuoteAuthorTitle, useLabelAsPlaceholder = true, maxLines = 1, modifier = Modifier.fillMaxWidth().padding(bottom = spacing.space12)) }
                if (!draftClassicHomeEnabled) {
                    item {
                        Card(modifier = Modifier.fillMaxWidth().padding(bottom = spacing.space12)) {
                            WindowDropdownPreference(title = MLang.AppSettings.Interface.HomeWallpaperSourceTitle, summary = MLang.AppSettings.Interface.HomeWallpaperSourceSummary, items = listOf(MLang.AppSettings.Interface.HomeWallpaperSourceGallery, MLang.AppSettings.Interface.HomeWallpaperSourceUrl), selectedIndex = -1, onSelectedIndexChange = { index -> when (index) { 0 -> onLaunchGalleryPicker(); 1 -> showUrlInputDialog = true } })
                        }
                    }
                }
            }
...
private fun MoeHomeRemoteWallpaperUrlDialog(show: Boolean, initialUrl: String, onDismiss: () -> Unit, onConfirm: (String) -> Unit) {
    ...
    AppFormDialog(show = show, title = MLang.AppSettings.Interface.HomeWallpaperUrlDialogTitle, onDismissRequest = onDismiss, onConfirm = { val trimmed = url.trim(); if (trimmed.isNotEmpty()) onConfirm(trimmed) }, scrollable = false) {
