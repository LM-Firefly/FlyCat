package com.github.yumelira.yumebox.feature.settings.presentation.util
...
internal object MetaWebDavBackup {
    ...
    suspend fun backup(context: Context, config: MetaWebDavConfig, repository: BackupDataSource): Result<String> = withContext(Dispatchers.IO) {
        runCatching {
            val client = WebDavClient(config)
    ...
    suspend fun restoreLatest(context: Context, config: MetaWebDavConfig, repository: BackupDataSource): Result<String> = withContext(Dispatchers.IO) {
        runCatching {
            val client = WebDavClient(config)
            client.ensureRootDirectory()
