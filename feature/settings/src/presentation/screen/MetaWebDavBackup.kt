...
internal object MetaWebDavBackup {
    ...
    suspend fun backup(context: Context, config: MetaWebDavConfig, repository: com.github.yumelira.yumebox.feature.settings.presentation.backup.BackupRepository): Result<String> = withContext(Dispatchers.IO) {
        runCatching {
            val client = WebDavClient(config)
            client.ensureRootDirectory()
            val tempFile = File(context.cacheDir, repository.defaultBackupFileName())
            try {
                repository.exportBackup(tempFile.outputStream())
    ...
    suspend fun restoreLatest(context: Context, config: MetaWebDavConfig, repository: com.github.yumelira.yumebox.feature.settings.presentation.backup.BackupRepository): Result<String> = withContext(Dispatchers.IO) {
        runCatching {
            val client = WebDavClient(config)
            client.ensureRootDirectory()
            ...
            try {
                ...
                repository.restoreBackup(tempFile.inputStream())
                latest.name
            } finally {
                tempFile.delete()
            }
