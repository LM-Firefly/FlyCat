...
internal fun submitProfile(draft: ProfileDraft) {
    ...
    if (draft.typeIndex == PROFILE_IMPORT_TYPE_URL && draft.url.isBlank()) {
        actions.showError(FlyTxt.ProfilesPage.Validation.EnterUrl)
        return
    }
    if (draft.typeIndex == PROFILE_IMPORT_TYPE_FILE && draft.filePath.isBlank()) {
        actions.showError(FlyTxt.ProfilesPage.Validation.SelectFile)
        return
    }
    ...
    if (profile != null) {
        ...
        actions.addProfile(
            draft.name.ifBlank { FlyTxt.ProfilesPage.Input.NewProfile },
            draft.url,
            Profile.Type.Url,
            0L,
            null,
