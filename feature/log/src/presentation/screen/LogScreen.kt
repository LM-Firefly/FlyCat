...
fun LogScreen(navigator: Navigator) {
    ...
    LifecycleEventEffect(androidx.lifecycle.Lifecycle.Event.ON_RESUME) { logScreenResumed = true }
    LifecycleEventEffect(androidx.lifecycle.Lifecycle.Event.ON_PAUSE) { logScreenResumed = false }
            ...
            Column(
                ...
                FloatingActionButton(
                    ...
                    Icon(
                        ...
                        contentDescription =
                            if (isRecording) MLang.Log.Action.StopRecording else MLang.Log.Action.StartRecording,
                        tint = MiuixTheme.colorScheme.onPrimary,
                    )
                }
                FloatingActionButton(
...
fun LogDetailScreen(
    ...
    LifecycleEventEffect(androidx.lifecycle.Lifecycle.Event.ON_RESUME) { detailResumed = true }
    LifecycleEventEffect(androidx.lifecycle.Lifecycle.Event.ON_PAUSE) { detailResumed = false }
    ...
    Scaffold(
        ...
        floatingActionButton = {
            Column(
                ...
                ) { Icon(imageVector = MiuixIcons.Download, contentDescription = MLang.Log.Action.Save, tint = MiuixTheme.colorScheme.onPrimary) }
                FloatingActionButton(onClick = { viewModel.deleteLogFile(fileName); navigator.navigateUp() }) { Icon(imageVector = MiuixIcons.Delete, contentDescription = MLang.Log.Action.Delete, tint = MiuixTheme.colorScheme.onPrimary) }
            }
        },
    ) { innerPadding ->
        when {
            isLoading -> { CenteredText(firstLine = MLang.Log.Detail.Loading, secondLine = "") }
            logEntries.isEmpty() -> { CenteredText(firstLine = if (isCurrentFileRecording) MLang.Log.Detail.WaitingLog else MLang.Log.Detail.LogEmpty, secondLine = if (isCurrentFileRecording) { MLang.Log.Detail.WillShowWhenGenerated } else { MLang.Log.Detail.NoLogContent }) }
            else -> {
    ...
    AnimatedVisibility(visible = visible, enter = fadeIn(animationSpec = tween(300)) + slideInVertically(animationSpec = tween(300), initialOffsetY = { -it / 2 })) {
        Card(modifier = Modifier.padding(vertical = 4.dp).pressable(interactionSource = interactionSource, indication = SinkFeedback()).clickable(interactionSource = interactionSource, indication = null, onClick = onClick)) {
            Row(modifier = Modifier.fillMaxWidth().padding(horizontal = 12.dp, vertical = 10.dp), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.SpaceBetween) {
                ...
                if (fileInfo.isRecording) { Text(text = MLang.Log.Status.Recording, style = MiuixTheme.textStyles.body2, color = MiuixTheme.colorScheme.primary, modifier = Modifier.padding(start = 8.dp)) }
            }
        }
    }
}
