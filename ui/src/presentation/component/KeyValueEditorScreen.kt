...
fun StringListEditorScreen(navigator: Navigator) {
    ...
    EditorScaffold(
        ...
        actions = remember(isOverrideRuleEditor) {
            listOf(
                EditorAction(icon = MiuixIcons.Reset, contentDescription = "Reset", onClick = { dialogState = StringListDialogState.Reset }),
                EditorAction(icon = Yume.`Badge-plus`, contentDescription = "Add", onClick = { dialogState = if (isOverrideRuleEditor) { StringListDialogState.AddRule } else { StringListDialogState.Add } }),
            )
        },
    ) { innerPadding ->
...
fun KeyValueEditorScreen(navigator: Navigator) {
    ...
    EditorScaffold(
        ...
        actions =
            remember {
                listOf(
                    EditorAction(icon = MiuixIcons.Reset, contentDescription = "Reset", onClick = { dialogState = KeyValueDialogState.Reset }),
                    EditorAction(icon = MiuixIcons.AddCircle, contentDescription = "Add", onClick = { dialogState = KeyValueDialogState.Add }),
                )
            },
            navigationIconPadding = 0.dp,
    ) { innerPadding ->
