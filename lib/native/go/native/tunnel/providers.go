...
func QueryProviders() []*Provider {
	...
	for _, p := range providers {
		...
		// Always extract format from JSON (needed for rule providers)
		if raw, err := json.Marshal(p); err == nil {
			var fmtData struct {
				Format string `json:"format"`
			}
			if err2 := json.Unmarshal(raw, &fmtData); err2 == nil && fmtData.Format != "" {
				item.Format = fmtData.Format
			}
		}
		// Extract subscription info from cache or JSON
		if cached := cachefile.Cache().GetSubscriptionInfo(p.Name()); cached != "" {
			...
			if raw, err := json.Marshal(p); err == nil {
				...
				if err2 := json.Unmarshal(raw, &data); err2 == nil {
					...
					if data.AgeSecretKey != "" {
						item.AgeSecretKey = data.AgeSecretKey
					}
				} else {
