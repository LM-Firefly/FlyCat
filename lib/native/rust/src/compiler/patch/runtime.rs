...
pub fn patch_static_runtime(root: &mut JsonValue, profile_dir: &Path, run_mode: RunMode) {
    ...
    // `system://` is deliberately NOT appended here: that nameserver resolves through `dns.UpdateSystemDNS`, which only an in-process host can call.
    backfill_enabled_dns_without_nameserver(object);

    // Force tun block off — EXCEPT in tun mode, where the compiled tun block is authoritative.
...
/// An override may force `dns.enable: true` onto a profile that carries no nameservers.
