...
/// Rewrites `<escaped>` keys back to their literal form, in place and without reallocating when the subtree contains no escaped key (the overwhelmingly common case).
