...
fn built_in_direct_rules_stack_after_built_in_global_rules() {
    ...
    let overrides = vec![
        LoadedOverride {
            ...
            ext: "yaml".to_string(),
            content: include_str!(
                "../../../../../data/assets/overrides/builtin/pudding_dog.yaml"
            )
            .to_string(),
        },
        LoadedOverride {
    ...
    assert_eq!(rules.last().and_then(JsonValue::as_str), Some("MATCH,PROXY"));
    assert!(
        rules
            .iter()
...
fn compile_request_preserves_two_override_file_chain_order() {
    let temp_dir = std::env::temp_dir().join(format!(
        "yumebox-two-override-chain-test-{}",
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .expect("system time before unix epoch")
            .as_nanos()
    ...
    request.overrides = vec![
        crate::model::OverrideSpec {
            path: global_path.to_string_lossy().into_owned(),
            ext: "yaml".to_string(),
        },
        crate::model::OverrideSpec {
            path: append_path.to_string_lossy().into_owned(),
            ext: "yaml".to_string(),
        },
    ];

    let result = compile_request(request, false).expect("compile ordered override files");
    let root: JsonValue = serde_yaml::from_str(&result.final_yaml).expect("parse final yaml");
    let rules = root["rules"].as_array().expect("rules");
...
"#
    ...
    assert_eq!(providers["base"]["type"], JsonValue::String("http".to_string()));
    assert_eq!(providers["base"]["interval"], JsonValue::from(3600));
    assert_eq!(providers["extra"]["path"], JsonValue::String("extra.yaml".to_string()));
}
