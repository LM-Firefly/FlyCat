...
///
/// Creating a `boa_engine::Context` and evaluating the helper script dominates multi-JS chains.
...
fn wrap_override_script(content: &str) -> String {
    // Evaluate the user script inside an IIFE so `function main` / `var` do not leak across
    // reused realms, then return the main function for a direct native call.
