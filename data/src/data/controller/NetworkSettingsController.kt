...
class NetworkSettingsController(
    ...
    override fun setProxyMode(mode: ProxyMode) {
    ...
    override suspend fun startService(mode: ProxyMode): Result<Unit> = runCatching {
...
class NetworkSettingsCommandExecutor(private val store: NetworkSettingsStore, private val restartProxy: suspend (ProxyMode) -> Unit, private val beforeRestart: suspend (ProxyMode) -> Unit = {}) {
    suspend fun startService(mode: ProxyMode): Result<Unit> = runCatching { store.proxyMode.set(mode); beforeRestart(mode); withContext(Dispatchers.IO) { restartProxy(mode) } }
    suspend fun restartConfiguredService(): Result<Unit> { val targetMode = store.proxyMode.value; return startService(targetMode) }
}
