...
class AppTrafficStatisticsCollector(
    ...
    private fun startTrafficMonitoring(parentScope: CoroutineScope): Job {
        return parentScope.launch {
            ...
            trafficTotalFlow.collectLatest { traffic ->
                runCatching { collectTrafficData(TrafficData.from(traffic)) }.onFailure { error ->
    ...
    private suspend fun collectTrafficData(totalTraffic: TrafficData) {
        ...
        val trafficReset = !isFirstCall && !profileChanged && !baselinesEmpty && (totalTraffic.upload < lastTotalUpload || totalTraffic.download < lastTotalDownload)
        if (isFirstCall || profileChanged || baselinesEmpty || trafficReset) {
            if (baselinesEmpty) { recordUnattributedDeltaIfNeeded(timestamp = timestamp, currentUpload = totalTraffic.upload, currentDownload = totalTraffic.download) }
            initializeTotals(
        ...
        if (trafficDeltas.isNotEmpty()) {
            pendingDeltas.addAll(trafficDeltas.values)
        }
    ...
    private fun collectConnectionDeltas(
        ...
        if (connectionBaselines.size > MAX_CONNECTION_BASELINES) {
            val iterator = connectionBaselines.iterator()
            var toRemove = connectionBaselines.size - MAX_CONNECTION_BASELINES
            while (iterator.hasNext() && toRemove > 0) { iterator.next(); iterator.remove(); toRemove-- }
        }
    ...
    private fun recordUnattributedDeltaIfNeeded(
        ...
        pendingDeltas.add(TrafficStatisticsBuckets.buildUnattributedRecord(uploadDelta = uploadDelta, downloadDelta = downloadDelta))
    }
    ...
    companion object {
        ...
        private const val MAX_CONNECTION_BASELINES = 2048
