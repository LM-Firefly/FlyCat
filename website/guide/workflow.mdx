...
FlyCat 负责管理配置、覆写和运行会话；mihomo 负责解析节点、建立连接并提供运行数据.

## 总体架构

这张图展示 FlyCat 从配置入口到运行数据的完整架构：配置、覆写、会话、核心、系统接入与观测数据彼此如何连接.

<Frame><img src="/images/diagrams/flycat-architecture.svg" alt="FlyCat 全部功能的架构关系图" noZoom={true} /></Frame>
...
在 **Vpn Service** 模式中，运行时补丁会关闭上述 Tun 入口；**Tun** 和 **eBPF** 模式都保留 profile 自己的配置.

## 自定义覆写与热重载

自定义覆写保存后，FlyCat 会重新应用当前配置正在使用的覆写链.运行中的配置不会修改订阅原文；重新加载失败时，运行会话会尝试恢复上一份有效配置.
...
三种模式共享配置编译流程，但接管流量的宿主不同：**Vpn Service** 使用 Android VPN 服务，**Tun** 使用 Root mihomo 进程，**eBPF** 使用 Root mihomo 内置的 eBPF listener.
