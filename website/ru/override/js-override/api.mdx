...
icon: terminal
---
Переопределения JavaScript выполняются во встроенной среде JavaScript FlyCat. Следующие API представляют собой полные глобальные методы, предоставляемые средой выполнения.
...
| `Buffer.isBuffer(value)` | Определите, является ли это объектом буфера FlyCat. |
...
function main(profile) {
  const buffer = Buffer.from("FlyCat", "utf8");
