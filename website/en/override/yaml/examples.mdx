...
FlyCat protects the string value of `reality-opts.short-id` in the top-level `proxies` list. This way `0123` and `1e5` will not be parsed as numbers by YAML.
...
After the user overwrite is completed, FlyCat will still perform runtime patches:
