...
<Columns cols={3}>
  ...
  <Card title="Submit an issue" icon="github" href="https://github.com/LM-Firefly/FlyCat/issues/new/choose">
...
<Steps>
  ...
  <Step title="Distinguish the runtime mode">
    Test the configuration with `VPN Service` first. If only Root `Tun` fails, check Root authorization, SELinux, and routing.
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="HTTP 401 / 403 / 404 / 5xx" icon="cloud-alert">
    The subscription or external provider returned an error status. FlyCat does not treat an HTTP error page as a valid subscription configuration.
  ...
  <Accordion title="Provider download failed / external proxies are empty" icon="cloud-off">
    FlyCat prefetches `proxy-providers` and `rule-providers` during import. The core also fetches missing resources after it starts. If a download fails, check the following fields:
  ...
  <Accordion title="Compiled provider path escaped profile scope" icon="shield-alert">
    ...
    Remove absolute paths, `../`, legacy `./ruleset/` paths, and paths containing `/clash/` from overrides. Let FlyCat generate an isolated path from the provider name.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="VPN permission denied / Establish VPN rejected by system" icon="shield-x">
    ...
    - Revoke FlyCat's VPN permission in system settings, then grant it again.
    - If the device vendor restricts background startup, allow FlyCat to run in the background.
  </Accordion>

  <Accordion title="root core launch failed" icon="badge-alert">
    ```text Root startup error
    root core launch failed (success=false ...)
    Tun requires usable Root permission for FlyCat
    ```

    The `Tun` runtime mode requires a working `su`. Confirm that the Root manager has authorized the currently installed package and that permission is not set to "Only once." After an update or signing-key change, the Root manager may treat the app as a new client and require authorization again.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="The proxy protocol is shown as Unknown" icon="circle-help">
    ...
    If only a new protocol is shown as `Unknown`, record the `type` returned by `/providers/proxies` or export logs, then submit an issue with the FlyCat and core versions. Do not include proxy credentials.
  </Accordion>
  ...
  <Accordion title="Connected, but the network is unavailable" icon="wifi-off">
    <Steps>
      ...
      <Step title="Check Geo data">
        Before using an external APK, install a builtin APK once to write the Geo resources. Later external APK updates retain those files.
      </Step>
    </Steps>
  </Accordion>

  <Accordion title="What is the difference between external and builtin Geo?" icon="database">
    An `external` APK does not contain Geo data, so install a `builtin` APK once before first use. A `builtin` APK contains GeoIP, GeoSite, ASN, and BundleMRS data and is larger. With app data retained, later updates can use an `external` APK directly. Both use the same app features and core code.
  </Accordion>

  <Accordion title="How do I submit an actionable issue report?" icon="message-square-code">
    Include at least the following information:

    <ResponseField name="FlyCat version" type="string" required>
    ...
    <ResponseField name="Runtime mode" type="VPN Service | Tun" required>
