/*
 * Android native payload extractor — single-threaded JNI context.
 * MISRA-C deviations are intentional for FFI/JNI interop.
 * strerror() is safe here because this runs on a single thread.
 */
// cppcheck-suppress misra-c2012-17.3,misra-c2012-15.4,misra-c2012-15.5,misra-c2012-21.14
// cppcheck-suppress cstyleCast,variableScope
#include <jni.h>
#include <lzma.h>
#include <errno.h>
#include <fcntl.h>
...
/* nosemgrep: cpp.lang.security.insecure-use-string-fprintf.insecure-use-string-fprintf */
/* Format strings are always compile-time constants; vsnprintf is bounded by capacity. */
...
static void sha256_update(Sha256 *sha, const uint8_t *data, size_t size) {
    ...
    while (size > 0) {
        ...
        /* nosemgrep: cpp.lang.security.insecure-use-memcpy.insecure-use-memcpy */
        /* count is bounded by available = sizeof(block) - block_size; no overflow possible. */
...
static NativeArchive *open_native_archive(
    ...
    /* nosemgrep: cpp.lang.security.file-system.insecure-temp-file.insecure-temp-file */
    /* APK path is from JNI (app's own package); O_CLOEXEC prevents fd leak. */
...
static bool extract_payload(
    ...
    /* nosemgrep: cpp.lang.security.file-system.insecure-temp-file.insecure-temp-file */
    /* O_EXCL ensures atomic creation; O_CLOEXEC prevents fd leak; 0600 restricts perms. */
...
Java_dev_flycat_loader_NativePayloadExtractor_openArchive(
        JNIEnv *env,
        jclass type,
    ...
    /* nosemgrep: cpp.lang.security.insecure-use-string-fprintf.insecure-use-string-fprintf */
    /* GetStringUTFChars returns null-terminated JNI strings; not attacker-controlled. */
...
Java_dev_flycat_loader_NativePayloadExtractor_extract(
        JNIEnv *env,
        jclass type,
        jlong archive_value,
...
__attribute__((visibility("default"))) JNIEXPORT void JNICALL
Java_dev_flycat_loader_NativePayloadExtractor_closeArchive(
        JNIEnv *env,
        jclass type,
        jlong archive_value) {
    (void)env;
