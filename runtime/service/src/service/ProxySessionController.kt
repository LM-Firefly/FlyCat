...
internal class ProxySessionController(
    ...
    fun onStartCommand() {
        if (notificationJob?.isActive != true) {
            notificationJob = notificationManager.startTrafficUpdate(
                scope = scope,
                screenOn = powerController.screenOn,
                appForeground = powerController.appForeground,
                queryTrafficNow = { if (isRuntimeInitialized) runtime.queryTrafficNow() else 0L },
                queryTrafficTotal = { if (isRuntimeInitialized) runtime.queryTrafficTotal() else 0L },
            )
        }
    }
    ...
    fun onDestroy() {
        ...
        if (isRuntimeInitialized) {
            ...
            thread(name = "$logTag-runtime-destroy") { runtime.destroy() }
        }
