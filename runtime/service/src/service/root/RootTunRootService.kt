...
class RootTunRootService : RootService() {
    ...
    private val binder =
        object : IRootTunService.Stub() {
            ...
            override fun restartRootTun(requestJson: String): String {
                ...
                return binderTimed { runtime.restart(createSpec("restart")) }?.let { encodeResult(it) } ?: encodeTimeoutResult()
            }
    ...
    /** Wraps a suspend call with [BINDER_CALL_TIMEOUT_MS] timeout. Returns null on timeout. */
    private fun <T> binderTimed(block: suspend () -> T): T? = runBlocking { withTimeoutOrNull(BINDER_CALL_TIMEOUT_MS) { block() } }
}
