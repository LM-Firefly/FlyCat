...
/**
 * Builds the built-in `tproxy-port` + `iptables` fragment for TPROXY mode. mihomo opens the tproxy listener, programs and tears down the host iptables mangle rules itself (`iptables.enable: true`) — no Kotlin-side iptables.
 * Access-control uids flow through `iptables.include-uid`/`exclude-uid` (mihomo excludes the core's own uid so its egress never loops). DNS is left to the compiler, as in Tun.
 */
object TproxyOverride {
    ...
    fun buildYaml(config: TproxyConfig): String {
        ...
        val dns =
            linkedMapOf<String, Any?>(
                "enable" to true,
                // dns-redirect needs a concrete listen address (mihomo ParseAddrPort's it and fails the whole iptables setup if empty); the REDIRECT rule points :53 traffic at this port.
        ...
        // No allow-lan here: the tproxy listener binds 0.0.0.0 on its own (mihomo ReCreateTProxy), so the other inbounds (http/socks/dns) stay on 127.0.0.1 and are not exposed to the LAN.
