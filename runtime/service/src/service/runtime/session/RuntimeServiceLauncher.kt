...
object RuntimeServiceLauncher : LocalRuntimeServiceContract {
    ...
    fun start(context: Context, mode: RunMode, source: String = ProxyServiceContracts.SOURCE_UNKNOWN) {
        require(mode == RunMode.Vpn) { "RuntimeServiceLauncher only supports VpnService mode; use RootTunServiceBridge for root modes" }
    ...
    fun stop(context: Context, mode: RunMode) {
        require(mode == RunMode.Vpn) { "RuntimeServiceLauncher only supports VpnService mode" }
