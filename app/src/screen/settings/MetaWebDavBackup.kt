...
internal object MetaWebDavBackup {
    ...
    suspend fun backup(context: Context, config: MetaWebDavConfig, repository: com.github.yumelira.yumebox.screen.settings.backup.BackupRepository): Result<String> = withContext(Dispatchers.IO) {
        runCatching {
            val client = WebDavClient(config)
    ...
    suspend fun restoreLatest(context: Context, config: MetaWebDavConfig, repository: com.github.yumelira.yumebox.screen.settings.backup.BackupRepository): Result<String> = withContext(Dispatchers.IO) {
        runCatching {
            val client = WebDavClient(config)
            client.ensureRootDirectory()
