...
class App : Application() {
    ...
    override fun onCreate() {
        ...
        // Permanent mihomo log → AppLogBuffer subscription (lives for the entire process).
        // Uses applicationScope (Koin-managed) instead of startupScope so it survives activity recreation and is properly tracked by the DI lifecycle.
        val applicationScope = koin.get<CoroutineScope>(named(APPLICATION_SCOPE_NAME))
        runCatching {
            val channel = Clash.subscribeLogcat()
            applicationScope.launch {
                for (msg in channel) {
                    val priority = when (msg.level) {
        ...
        }.onFailure { Timber.w(it, "Mihomo log buffer subscription skipped") }
