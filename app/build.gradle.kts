        ...
        onVariants { variant ->
            variant.outputs.forEach { output ->
                ...
                (output as com.android.build.api.variant.impl.VariantOutputImpl).outputFileName.set(
                    if (withExtension) {
                        "${providers.gradleProperty("project.name").get()}_Extension-${appVersionName}-${abiName}-${buildTypeName}.apk"
                    } else {
                        "${providers.gradleProperty("project.name").get()}-${appVersionName}-${abiName}-${buildTypeName}.apk"
                    }
                )
            }
        }
