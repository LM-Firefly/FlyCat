...
abstract class PackApkTask : DefaultTask() {
    ...
    @get:Input
    abstract val originalApplication: Property<String>

    @get:Input
    abstract val originalComponentFactory: Property<String>
    ...
    protected fun packApk(input: File, output: File) {
        ...
        val dexPayload = ZipFile(input).use { zip ->
        ...
        val nativePayload = ZipFile(input).use { zip ->
            zip.entries().asSequence()
        ...
        runCommand(
            listOf(tools.resolve(executable("zipalign")).absolutePath, "-P", "16", "-f", "4", unaligned.absolutePath, aligned.absolutePath),
    ...
    private fun payloadId(dexEntries: List<PayloadEntry>, nativeEntries: List<NativePayloadEntry>): String {
    ...
    private fun nativePayload(zip: ZipFile, entry: ZipEntry): NativePayloadEntry {
        val match = NATIVE_LIBRARY.matchEntire(entry.name) ?: error("Invalid native library: ${entry.name}")
        ...
        val originalBytes = if (alreadyCompressed) {
            org.tukaani.xz.XZInputStream(packagedBytes.inputStream()).use { it.readBytes() }
        } else {
            packagedBytes
        }
        val outputName = if (alreadyCompressed) {
    ...
    private fun runCommand(command: List<String>, failure: String, environment: Map<String, String> = emptyMap()) {
        val process = ProcessBuilder(command).apply { environment().putAll(environment) }.inheritIO().start()
    ...
    private companion object {
        ...
        //  - libclash.so is the mihomo PIE core — it is fork+exec'd out of process, and a non-root app
