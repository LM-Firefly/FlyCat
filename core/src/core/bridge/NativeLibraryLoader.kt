...
/**
 * 当应用以 -Pnativelibs.compress=true 打包时（参见 NativeLibCompressor / app build.gradle.kts），加载三个核心原生库。
 * APK 打包器会将每个库移动到 XZ 压缩的 assets/loader/lib/<abi>/<name>.so.xz 载荷中，因此必须将这些二进制文件解压到应用私有存储，再通过绝对路径加载。
 *
 * 在 APK 打包器运行之前，-Pnativelibs.compress=true 会将三个核心二进制文件以 lib/<abi>/<base>.xz.so 的形式存储。
 ...
 * 当 libbridge 解析 DT_NEEDED libmihomo.so 以及后续打开 liboverride.so 时，必须能通过名称找到它们。
 *
...
object NativeLibraryLoader {
    ...
    // Dependency order: liboverride first, then libmihomo before libbridge (its DT_NEEDED).
    // Kept in sync with app build.gradle.kts and NativeLibCompressor.coreLibs.
    private val CORE_LIBS = listOf("liboverride.so", "libmihomo.so", "libbridge.so")
