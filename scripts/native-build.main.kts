...
class NdkTools(private val config: ProjectConfig) {
    private val sdkDir: File by lazy {
        val rawPath = System.getenv("ANDROID_HOME")
    ...
    private fun applyKernelPatches() {
        ...
        basePatches.forEach { patch ->
            ...
            val alreadyApplied = executeCommand(
                command = listOf("git", "apply", "--reverse", "--check", "--ignore-whitespace", effectivePatch.absolutePath),
            ...
            val result = executeCommand(
                command = listOf("git", "apply", "--ignore-whitespace", effectivePatch.absolutePath),
