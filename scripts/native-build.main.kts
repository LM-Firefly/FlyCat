...
class NativeLibCompressor(private val config: ProjectConfig) {
    ...
    private val coreLibs = listOf("liboverride.so", "libmihomo.so")
...
fun cleanStaleBridgeLibs(config: ProjectConfig) {
    val appJniRoot = File("jniLibs")
    val outputDir = File("build/native/cpp")
    val abis = config.getCsv("abi.app.list", "armeabi-v7a,arm64-v8a,x86,x86_64")
    println("[C++] JNI bridge migrated to Rust; --cpp is now a no-op. Cleaning stale libbridge.so artifacts.")
    abis.forEach { abi ->
        File(appJniRoot, "$abi/libbridge.so").delete()
        File(outputDir, "obj/$abi/libbridge.so").delete()
    }
}
